...
%100 ms ごとに data acquisition できる？
...
dataall = zeros(n2,3);
for i = 1:n
    dataall(:,1) = event.Data(:,1)*100; %(mV), Axoclamp2B からの出力は 10*Vm なので *1000(mV)/10(gain)
    dataall(:,2) = event.Data(:,2)*10; %(nA), V->I への変換必要 10 /(*0.1) mV/nA = 100 mV/nA: 1V で 10 nA
    dataall(:,3) = event.Data(:,3)*1000; %(mV), phot sensor の出力は V なので mV に変換
